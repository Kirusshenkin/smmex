...
var EmojiPicker = function(selector){
    var self = {};

    self.selector = selector;
    self.holder = $(selector);

    self.area = self.holder.find('textarea');
    self.panel = self.holder.find('.emoji-panel');
    self.panel_tabs = self.panel.find('.emoji-panel-tabs');
    self.panel_list = self.panel.find('.emoji-panel-list');

    self.init = function(){
        self.panel.on('click', '.emoji-panel-header[data-category]', self.category);
        self.panel.on('click', '.emoji-panel-control', self.toggle);
        self.panel.on('click', '.emoji-panel-item', self.emoji);
    }

    self.toggle = function(e){
        e.preventDefault();

        $(this).find('i').toggleClass('fa-smile');
        $(this).find('i').toggleClass('fa-times');
        self.panel.toggleClass('collapsed');

        self.panel_tabs.find('.emoji-panel-header[data-category]:first').click();
    }

    self.emoji = function(e){
        e.preventDefault();

        insertTextAtCursor(document.querySelector(selector + ' textarea'), $(this).html());
        $(selector + ' textarea').trigger('keyup');
    }
    ...
    self.category = function(){
        ...
        var icons = self.getIcons(category);

        $.each(icons, function(key, item){
            var icon = document.createElement('span');
            icon.className = 'emoji-panel-item';
            icon.innerHTML = item;

            html += icon.outerHTML;
        })

        self.panel_list.html(html);

    }

    self.getIcons = function(catId){
        var icons = {};
        Config.EmojiCategories[catId].forEach(function(item){
            var dataItem = Config.Emoji[item];
            var name = dataItem[1][0];
            icons[name] = dataItem[0];
        })
